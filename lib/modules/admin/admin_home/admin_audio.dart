...
class AdminAudio extends StatelessWidget {
  const AdminAudio ({Key? key}) : super(key: key);
            ...
            child: Column(
              children: [
                Expanded(child: ListView.separated(itemBuilder: (context,index)=>InkWell(
                  onTap: (){
                    var audioUrl=AdminLayoutCubit.get(context).audioAdmin[index].video;
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Column(
                            children: [
                              Text('${AdminLayoutCubit.get(context).audioAdmin[index].name}'),
                              Row(
                                children: [
                                  IconButton(onPressed: (){
                                    var postId=AdminLayoutCubit.get(context).audioAdminId[index];
                                    Navigator.push(context, MaterialPageRoute(builder: (context)=>CommentUserPage(postId.toString(),'Audio')));
                                  }, icon: Icon(Icons.chat)),
                                  IconButton(onPressed: (){
                                    var postId=AdminLayoutCubit.get(context).audioAdminId[index];
                                    AdminLayoutCubit.get(context).deletePost(postId, 'Audio', (){
                                      AdminLayoutCubit.get(context).audioAdmin.clear();
                                    }, (){AdminLayoutCubit.get(context).getAudio();});
                          ...
                          Column(
                            children: [
                              Text('time: ${DateFormat('HH:mm').format(DateTime.parse(AdminLayoutCubit.get(context).audioAdmin[index].dateTime.toString()))}',style: TextStyle(color: Colors.grey),),
                              Text('date: ${DateFormat('MM-dd').format(DateTime.parse(AdminLayoutCubit.get(context).audioAdmin[index].dateTime.toString()))}',style: TextStyle(color: Colors.grey)),
                    ...
                    , separatorBuilder: (context,index)=>
                        SizedBox(height: 12,)
                    , itemCount: AdminLayoutCubit.get(context).audioAdmin.length))
              ],
            ),
          ),
        );
